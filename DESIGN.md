...
- **Hero cube**: tapping cycles through three shapes — cube, sphere, torus — all rendered as flat bordered planes inside one `preserve-3d` wrapper, cross-fading **per element** (opacity on the wrapper itself would flatten its 3D children). The cube is the six-plane box (`CUBE_FACES`). The sphere is six great circles sharing one rotation axis (`SPHERE_RINGS`), which is the standard way to fake a wireframe globe out of flat rings — they all pass through the same two pole points. The torus is a ring of small tube circles spaced evenly around a circular path via `rotateY(angle) translateZ(majorRadius)` (`TORUS_RINGS`); each ring keeps the orientation from its own rotateY step, which is what reads as a donut rather than a flat fan once the object tilts.
